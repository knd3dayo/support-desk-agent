...
- InvestigateAgent とは分けて、通常回答ドラフトとエスカレーション問い合わせ文案の責務を分離する
- 顧客向けに返す文案と、バックサポートへ渡す内部メモの境界を明確にする
- 実行クラスは [src/support_desk_agent/agents/back_support_inquiry_writer_agent.py](/home/user/source/repos/support-ope-agents/src/support_desk_agent/agents/back_support_inquiry_writer_agent.py) の BackSupportInquiryWriterPhaseExecutor とし、現時点では共有メモリ更新と問い合わせ文案組み立てを担当する
