...
1. 事実整合チェック
   調査結果と矛盾した記述がないかを見る。
2. 根拠文書検索
   DeepAgents backend の /policy/<source_name>/ 配下に mount した document_sources を参照し、社内規定、政府ガイドライン、法令の根拠候補を抽出する。返却結果の `summary` には生成要約ではなく raw snippet を保持する。
3. 表現・ポリシーチェック
...
- notice.required が false の場合、注意文不足だけでは差戻し理由にしない
