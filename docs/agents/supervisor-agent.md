...
IntakeAgent の出力を受けて調査方針を決定し、InvestigateAgent の結果を統合したうえで、承認フェーズへ進める回答案を管理する。
...
- 呼び出し先: InvestigateAgent、BackSupportEscalationAgent、BackSupportInquiryWriterAgent
...
SuperVisorAgent の処理は supervisor_subgraph 内で大きく次の 3 段階に分かれる。

1. 調査フェーズ管理
   IntakeAgent の出力と read_shared_memory で取得した共有メモリ内容を参照し、workflow_kind を基準にしつつ、workflow_kind が ambiguous_case で intake_category がより具体的な場合はそちらを優先して、InvestigateAgent の起動条件と再調査要否を決める。
   Intake 側で品質ゲートを通過している前提で、workflow_kind または intake_category の妥当性を解釈し、以降の調査方針と child agent 起動順を決める。
2. ドラフト作成フェーズ管理
   調査結果をもとに InvestigateAgent が生成した顧客向け回答ドラフトを確認し、承認フェーズへ進める。
3. エスカレーション管理
...
investigation フェーズでは、InvestigatePhaseExecutor を通してログ解析、文書検索、ドラフト生成を一体で実行し、結果を investigation summary と shared memory に反映する。
...
- 子エージェント起動の詳細は ToolRegistry と RuntimeService の責務に委ねる
...
- InvestigateAgent の draft_review 結果をどこまで構造化して保持するか
