...
4. 抜粋化
   SuperVisorAgent と DraftWriterAgent が採用判断しやすいよう、生成要約ではなく該当 Markdown 本文から raw snippet を返す。
...
- `search_documents` の builtin 実装は backend 共通 helper を使うが、結果 payload の組み立ては KnowledgeRetriever 側 caller で行う
