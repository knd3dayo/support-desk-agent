# ComplianceReviewerAgent ツール下書き
...
## role 固有ツール
- check_policy: Check compliance policy (provider=local, target=n/a, status=planned, override=allowed)
- request_revision: Request draft revision (provider=local, target=n/a, status=planned, override=allowed)
