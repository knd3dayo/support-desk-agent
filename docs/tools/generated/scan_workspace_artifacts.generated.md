# scan_workspace_artifacts ツール下書き

このファイルは ToolRegistry から半自動生成した下書きです。

## 概要
- description: Scan workspace artifacts

## 利用エージェント
- SuperVisorAgent: provider=local, target=n/a, status=planned, override=allowed
- BackSupportEscalationAgent: provider=local, target=n/a, status=planned, override=allowed

## 手編集メモ
- ここに入出力例、運用上の注意、MCP 接続前提などを追記する。
- docs/tools/specs/*.md の更新時に差分確認用の下書きとして使う。
