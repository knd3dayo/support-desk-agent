...
## role 固有ツール
- pii_mask: Mask API keys and similar secrets from issue text or logs (provider=builtin, target=configured-llm-pii-mask, status=implemented, override=allowed)
- classify_ticket: Classify customer support ticket with the configured LLM in PoC (provider=builtin, target=configured-llm-classify-ticket, status=implemented, override=allowed)
