...
- 論理ツール名と Python 実装ファイルは 1:1 対応ではない
...
- role 別ツール文書は廃止済みであり、以後は復活させない

## 4. 未実装の論理ツール

以下は docs/tools/specs 上では論理ツールとして扱うが、現時点では builtin 実装ファイルを持たず、ToolRegistry の not implemented handler で受けている。
