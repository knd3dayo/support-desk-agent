...
- 既定では未接続で、[config.yml](/home/user/source/repos/support-desk-agent/config.yml) の tools.ticket_sources.internal で接続先を構成する
...
- tools.logical_tools.internal_ticket は廃止済みで、設定しても validation error になる
...
- 入力は ticket_id を受け取る
- 接続先の MCP server 名や固定引数は tools.ticket_sources.internal に置く
...
- 既定実装なし
- tools.ticket_sources.internal の接続先構成が前提