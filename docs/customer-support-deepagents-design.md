...
- ObjectiveEvaluationAgent: 改善レポート生成時に state、shared memory、各 agent working memory を照合して客観評価する
...
IntakeAgent、ApprovalAgent、TicketUpdateAgent は LangGraph 上の疑似エージェントとして扱う。探索的な処理は各専門 agent が担い、改善レポート時は ObjectiveEvaluationAgent が workflow 終了後の state と memory を評価する。
...
- 改善レポート専用の評価主体として実装する
- SuperVisorAgent の判断をそのまま採用せず、CaseState、shared memory、各 agent の working memory、成果物一覧を照合する
- エージェント別点数、情報伝達監査、サブグラフ詳細シーケンスを含む客観評価レポートを生成する
- 詳細設計は [docs/agents/objective-evaluation-agent.md](/home/user/source/repos/support-ope-agents/docs/agents/objective-evaluation-agent.md) を参照する

#### IntakeAgent

- LangGraph の subgraph として実装する
- 問い合わせ受付時および継続問い合わせ受付時の定型前処理を担当し、必要に応じた PII マスキング、カテゴリ判定、初期メモ作成を行う
- subgraph 内では入力正規化、任意の PII マスキング、明示 ticket ID に基づく ticket 情報と添付ファイルの workspace 取り込み、分類、共有メモリ初期化を段階的に実行する
- 追加質問への回答や継続問い合わせが来た場合も IntakeAgent が再評価し、再度 Intake で完結すべきか、そのまま SuperVisorAgent へ再連携すべきかを判断する
...
- `search_documents` の builtin 実装では DeepAgents backend を直接使い、source ごとの `matched_paths`、`evidence`、`feature_bullets`、raw snippet を返す
- feature list 系の問い合わせでは source 名の明示一致を優先し、DraftWriterAgent が箇条書き回答へ落とし込みやすい形で返す
...
- 仕様問い合わせでは KnowledgeRetrieverAgent の source 結果から直接ドラフトを組み立て、feature bullets があれば箇条書き、根拠資料は Markdown リンクで提示する
- internal agent 名、共有メモリ文言、レビュー差戻し文面などの内部表現は顧客向けドラフトから除外する
...
- `check_policy` の builtin 実装では DeepAgents backend を直接使い、source ごとの raw snippet と evidence を返す
- 注意文チェックは `agents.ComplianceReviewerAgent.notice.required` が true の場合のみ必須とする
...
各エージェントは処理開始時に shared memory と自エージェントの working memory を確認し、既知事実、進捗、未完了タスクを踏まえて処理する。
...
共通指示には、shared memory と自エージェント working memory を処理開始時に必ず確認するルールを置く。
...
- ObjectiveEvaluationAgent: 専用の外部操作ツールは持たず、workflow state、shared memory、working memory、成果物一覧を読み取って評価する
- IntakeAgent: pii_mask, external_ticket, internal_ticket, classify_ticket, write_shared_memory
...
KnowledgeRetrieverAgent と ComplianceReviewerAgent の builtin 検索系 tool は、shared な検索ラッパーを介さず caller 側で結果を組み立てる。共通化は backend の mount / ignore pattern / candidate path 解決に限定し、回答精度に影響する snippet 抽出や結果整形は各 caller の責務とする。
...
- 改善レポートの合格閾値や memory 欠落時の減点値は agents.ObjectiveEvaluationAgent 配下へ置く
