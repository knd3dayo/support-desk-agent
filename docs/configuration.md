...
本書は [config.yml](/home/user/source/repos/support-ope-agents/config.yml) の主要設定方針、とくに KnowledgeRetrieverAgent の文書ソース、Agent 固有設定、論理ツール定義を説明する。
...
`tools.logical_tools` では、IntakeAgent と KnowledgeRetrieverAgent が共有する各論理ツールの有効化と供給元を指定する。
...
## 10. instruction override と tool docs 下書き
