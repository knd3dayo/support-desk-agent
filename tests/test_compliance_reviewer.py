    ...
    def test_check_policy_raises_when_llm_review_fails(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                ...
                tool = build_default_check_policy_tool(config)
                with self.assertRaisesRegex(RuntimeError, "LLM-backed policy review failed: ConnectionError: LLM connection failed"):
                    asyncio.run(tool(draft_response="この回答は必ず正しいです。", review_focus="注意文と断定表現を確認する"))

    def test_check_policy_logs_raw_response_when_llm_returns_non_json(self) -> None:
        class _NonJsonComplianceModel:
            async def ainvoke(self, messages):
                prompt = str(messages[0].content)
                if "Expand the review query" in prompt:
                    return type("_FakeResponse", (), {"content": '{"keywords": []}'})()
                return type("_FakeResponse", (), {"content": 'Internal Server Error'})()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "support_ope_agents.tools.default_check_policy._get_chat_model",
                return_value=_NonJsonComplianceModel(),
            ):
                tool = build_default_check_policy_tool(config)
                with self.assertLogs("support_ope_agents.tools.default_check_policy", level="ERROR") as captured:
                    with self.assertRaisesRegex(RuntimeError, "JSONDecodeError"):
                        asyncio.run(tool(draft_response="この回答は必ず正しいです。", review_focus="注意文と断定表現を確認する"))

            self.assertTrue(any("raw_response=Internal Server Error" in entry for entry in captured.output))
