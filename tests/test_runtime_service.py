...
class RuntimeServiceFlowTests(unittest.TestCase):
    ...
    def test_generate_support_improvement_report_writes_report_folder(self) -> None:
        ...
        self.assertLess(content.index("## Evaluator 評価観点一覧"), content.index("## Evaluator 指示上の評価方針"))
        self.assertLess(content.index("## Evaluator 指示上の評価方針"), content.index("## エージェント別評価"))
        ...
        self.assertIn("## Evaluator 指示上の評価方針", content)
        self.assertIn("## 評価方針", content.replace("## Evaluator 指示上の評価方針", "## 評価方針", 1))
        self.assertIn("質問内容を確認して", content)
