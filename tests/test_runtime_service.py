...
class RuntimeServiceFlowTests(unittest.TestCase):
    ...
    def test_generate_support_improvement_report_writes_report_folder(self) -> None:
        ...
        self.assertLess(content.index("## Evaluator 評価観点一覧"), content.index("## Evaluator チェックリスト評価"))
        self.assertLess(content.index("## Evaluator チェックリスト評価"), content.index("## エージェント別評価"))
        ...
        self.assertIn("## Evaluator チェックリスト評価", content)
        self.assertIn("### 標準チェックリスト", content)
        self.assertIn("### ユーザー指定チェックリスト", content)
        self.assertNotIn("## Evaluator 指示上の評価方針", content)
        ...
        self.assertIn("- 対応するチェックリスト項目: 回答に注意文が含まれているか", content)
